...
import { garuts } from '../../assets/FilmImg'
import Masonry, {ResponsiveMasonry} from 'react-responsive-masonry'
import { filmImages } from '../../constants/gallery'
...
const Film = () => {
  ...
  const imgAction = (action) => {
    let i = data.i;
    if(action === 'next-img') {
      setData({img: filmImages[i + 1], i: i + 1})
  ...
  return (
    <section className={`${styles.padding} w-full relative`}>
      ...
      <div>
        <img src={garuts} alt="garuts" className="object-contain" data-aos="fade" data-aos-duration="2000"/>
        <h1 className="font-shippori text-center text-[30px] text-[#373a3a] uppercase sm:mt-12 mt-6" data-aos="flip-up"> Film </h1>
      </div>

      <div className="sm:pt-12 pt-6">
        <ResponsiveMasonry columnsCountBreakPoints={{350: 1, 750: 2, 900: 3}}>
          <Masonry gutter="20px">
            {filmImages.map((image, i) => (
              <img 
                key={i}
                src={image} 
                alt={image}

...
import { logo, logo2, close, menu } from '../../assets/HomeImg';
      ...
      <div className={`${styles.paddingX} w-full flex justify-between items-center z-50`}>
        ...
        <div className="sm:flex hidden gap-5" data-aos="fade-down">
          <ul className="list-none sm:flex hidden justify-between items-center gap-5">
            {navLinks.map((nav) => (
              <li key={nav.id} className="font-montserrat uppercase font-medium cursor-pointer text-[13px]">
                <Link to={nav.link} className="hover:text-[#b4a194] text-[#40442f] ease-in-out duration-[.2s]">
                  {nav.title}
                </Link>
              </li>
            ))}
        ...
        <div className="sm:hidden flex flex-1 justify-end items-center z-50">
          ...
          <div className={`${toggle ? "flex" : "hidden"} h-screen flex-col justify-center items-center gap-[50px] p-6 
          bg-[#f7f5f5] absolute top-0 left-0 min-w-[80%] sidebar`}>
            ...
            <div className={`${styles.flexCenter}`}>
              <button type="button" className="h-[35px] w-[110px] bg-[#dac7ba] px-2 outline-none border-none">
